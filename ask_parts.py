...
## command
{input}
Answer the command with the given parts.
If possible, answer the command while using the given parts (BBa_).
It is not necessary to mention the content of parts that are not available.
""".strip()
...
def search_query(input_str, index_file):
    ...
    to_use = []
    used_title = []
    for _sim, body, title in filtered_samples[:6]:  # Get top 7 results
...
if __name__ == "__main__":
    query = "What is BBa_J435332?"  # ここに検索したいクエリを入力
    index_file = "igemindex4.pickle"  # Path of the Pickle file where the VectorStore object is stored
